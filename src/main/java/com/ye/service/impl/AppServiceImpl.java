...
//        );
    ...
    public Flux<String> chatGenerateCode(AppChatRequest request, Long loginUserId) {
        ...
        return streamHandlerExecutor.executeHandler(streamFlux, chatHistoryService, request.getId(), loginUserId,
                CodeGenTypeEnum.getEnumByValue(request.getCodeType()))
                .doFinally(signalType -> {
                    MonitorContextHolder.removeContext(); // 无论结果如何， 都删除当前的的监控上下文
                });
